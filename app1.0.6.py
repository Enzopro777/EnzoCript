...
def _build_header(ext: bytes, rsa_blob: bytes, chunk_size: int) -> bytes:
    """Construye y serializa la cabecera completa."""
    return (
        MAGIC
        + struct.pack("<H", VERSION_V2)
        + struct.pack("B", len(ext)) + ext
        + struct.pack("<H", len(rsa_blob)) + rsa_blob
        + struct.pack("<I", chunk_size)
    )

def _aad(idx: int, header: bytes) -> bytes:
    """Vincula criptográficamente cada chunk a la cabecera completa."""
    return header + struct.pack("<I", idx)
...
def _verify_chunks(path, offset, chunk_size, aesgcm, header):
    """Primera pasada: autentica todos los chunks sin escribir nada.
    Lanza ValueError si cualquier chunk falla la verificación."""
    sz = os.path.getsize(path)
    with open(path, "rb") as f:
        f.seek(offset)
        idx = 0
        while True:
            ...
            ct    = f.read(ct_len)
            try:
                aesgcm.decrypt(nonce, ct + tag, _aad(idx, header))
            except Exception:
                raise ValueError(f"Integrity failed on chunk {idx}. Tampered?")
            idx += 1
            if idx > MAX_CHUNKS: raise ValueError("Too many chunks.")
...
def benchmark_speed(chunk_size: int) -> float:
    ...
    # Benchmark puro de rendimiento AES-GCM sin carga de AAD
...
def decrypt_file_v2(container_path, priv_key_path, passphrase, output_path,
    ...
    with open(container_path,"rb") as fin:
        ...
        # Una vez leídos y validados, reconstituimos la cabecera
        header = _build_header(ext.encode(), rsa_blob, cs)
        ...
        # ── Primera pasada: verificación criptográfica completa ──────────────
        # El archivo de salida NO existe todavía en este punto. Si un chunk
        # está corrupto, lanzará excepción aquí y abortará todo.
        _verify_chunks(container_path, data_offset, cs, aesgcm, header)

        # ── Segunda pasada: escritura — solo llega aquí si todo es válido ────
        base,_ = os.path.splitext(output_path)
        fp = output_path if output_path.lower().endswith(ext.lower()) else base+ext
        mbs = benchmark_speed(cs); t0 = time.perf_counter()
        idx = 0; processed = 0
        
        # IMPORTANTE: Rebobinamos el archivo de lectura original al inicio de los datos
        ...
        with open(fp,"wb") as fout:
            while True:
                ...
                # Ya no usamos try/except porque ya sabemos que el archivo es 100% íntegro
                pt = aesgcm.decrypt(nonce, ct+tag, _aad(idx, header))
...
def encrypt_file_v2(file_path, pub_key_path, output_path, chunk_size,
                    progress_cb=None, cancel_flag=None):
    pub = serialization.load_pem_public_key(open(pub_key_path, "rb").read())
    aes_key = secrets.token_bytes(32)
    rsa_blob = pub.encrypt(aes_key, _oaep())
    ext = os.path.splitext(file_path)[1].encode() or b".bin"
    if len(ext) > MAX_EXT_LEN:
        ext = ext[:MAX_EXT_LEN]

    header = _build_header(ext, rsa_blob, chunk_size)
    aesgcm = AESGCM(aes_key)
    ...
    try:
        with open(file_path, "rb") as fin, open(tmp_path, "wb") as fout:
            ...
            while True:
                ...
                ct_tag = aesgcm.encrypt(nonce, chunk, _aad(idx, header))
                ct = ct_tag[:-16]; tag = ct_tag[-16:]
                fout.write(struct.pack("<I", len(ct)))
                fout.write(nonce)
                fout.write(tag)
                fout.write(ct)
                processed += len(chunk); idx += 1
                if progress_cb:
                    elapsed = time.perf_counter() - t0
                    speed = (processed / 1048576) / elapsed if elapsed > 0 else mbs
...
def decrypt_file_v1(container_path, priv_key_path, passphrase, output_path):
    ...
    try:
        ...
        # V1 usa None y por ende queda libre de errores con el nuevo _aad
